...
void UDungeonWorldBuilderComponent::BuildDungeon()
{
    ...
    for (int32 i = 0; i < DungeonMap.NbCellsX * DungeonMap.NbCellsY; i++)
    {
        const FCell& Cell = DungeonMap.GetCellRef(i);
        if (Cell.IsEmpty() || Cell.IsBanned())
            continue; // Skip empty or banned cells
        BuildCell(DungeonMap, Cell);
    }
...
void UDungeonWorldBuilderComponent::BuildCell(const FDungeonMap& DungeonMap, const FCell& Cell)
{
    const FIntCoordinate CellCoord = DungeonMap.GetCellWorldCoordinate(Cell.Index);
    DebugLog(FString::Printf(TEXT("Building cell at (%d, %d)"), CellCoord.x, CellCoord.y), this);
    FTransform CellTransform = FTransform::Identity;
    CellTransform.SetLocation(CellCoord.ToFVector());
    CellTransform.SetRotation(FQuat::Identity);
    CellTransform.SetScale3D(FVector::OneVector);
    AActor* SpawnedActor = GetWorld()->SpawnActor<APremadeRoom>(Cell.RoomClass, CellTransform.GetLocation(), CellTransform.GetRotation().Rotator());
    if (!SpawnedActor)
    {
